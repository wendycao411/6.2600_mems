...
def invert_layout(source, border=200, output_layer=MASK_LAYER):
    ...
    mask_source.flatten()  # collapse hierarchy before boolean for speed
    ...
    inverted = pg.boolean(
        ...
        operation="A-B",
        precision=1e-6,
        num_divisions=[4, 4],  # split into 4x4 spatial grid for speed
        layer=output_layer,
    )
...
def main():
    print("Building array layout...")
    source = array_module.make_array_4x4()
    print("Inverting layout (this may still take a minute)...")
